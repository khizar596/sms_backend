...
async def viewExaminationadmin():
    employees=[]
    cursor = col_employee.find({'role.0.name':'examadmin'})
    
    for document in cursor:
        role_name=document['role'][0]['name']
        if role_name=="examadmin":
            employees.append((ExaminationAdmin2(**document)))
    return employees

async def searchExaminationadmin(employee_id : str)->dict:

    document=  col_employee.find_one({"_id": ObjectId(employee_id),'role.0.name':'examadmin'},{'_id':0})
    if not document:
...
async def addExaminationadmin(details):
    ...
    role_relation= [colr.find_one({"_id": ObjectId(employe_role[0]),'name':'examadmin'},{'_id': 0})]
